...
//   const [selectedTopicIds, setSelectedTopicIds] = useState([]); // multiple topic ids
...
//   const [newQuestions, setNewQuestions] = useState([]); // array of new question objects

//   // For the inline new question form (for a single new question)
//   const [newQuestionData, setNewQuestionData] = useState({
//     questionText: "",
//     questionType: "MCQ", // or "TRUE_FALSE"
//     hint: "",
//     // For MCQ, options will be an array; for TRUE_FALSE, we can use fixed values
//     options: [""],
//     correctOptions: [],
//     topicId: "" // This dropdown will show only topics selected in quiz details.
//   });
...
//   useEffect(() => {
//     // Fetch existing questions for the educator
//     axios
...
//   // When visibility changes to PRIVATE, auto-generate a code.
//   useEffect(() => {
...
//       .then((response) => {
//         // Refresh topics list
//         return axios.get("http://localhost:8080/topics/getall-topics");
//       })
//       .then((res) => {
//         setTopics(res.data);
//         // Optionally, automatically add the new topic id to selectedTopicIds
//         setNewTopicName("");
...
//   // Filter existing questions based on selected difficulty and topics (only include questions whose topic id is in selectedTopicIds)
...
//     setNewQuestionData((prev) => ({ ...prev, options: [...prev.options, ""] }));
//   };

//   const toggleNewQuestionCorrectOption = (option) => {
...
//     }
//     setNewQuestionData((prev) => ({ ...prev, correctOptions: updatedCorrect }));
//   };

//   // Handler to add a new question to the quiz.
//   // Validate required fields. If valid, call the endpoint to create the new question.
...
//       difficulty, // inherit from quiz form
//       hint: newQuestionData.hint,
//       options: newQuestionData.questionType === "MCQ" ? newQuestionData.options : [], // For TRUE_FALSE, options can be fixed
//       correctOptions: newQuestionData.correctOptions,
//     };

//     // For the new question, we want a dropdown of topics that were selected in the quiz form.
...
//       return;
//     }
//     // Let the educator choose one topic from the available topics
...
//   // Total number of questions in quiz = existing + new
//   const numberOfQuestions = selectedExistingQuestionIds.length + newQuestions.length;

//   // Handler to submit the entire quiz creation form.
...
//     // Combine question IDs from existing and new questions.
//     const existingIds = selectedExistingQuestionIds;
//     const newIds = newQuestions.map((q) => q.id);
//     const allQuestionIds = [...existingIds, ...newIds];
...
//       numberOfQuestions,
//       difficulty,
//       topics: selectedTopicIds.join(","), // comma-separated
//       timeLimit,
//       questionIds: JSON.stringify(allQuestionIds),
//       visibility,
...
//   // Compute available topics for new questions: only those selected in quiz details.
//   const availableTopicsForNewQuestion = topics.filter((t) =>
//     selectedTopicIds.includes(t.id)
//   );
...
//                   />
//                 </div>
//                 {/* Topic selection for new question: only show topics selected in quiz */}
//                 <div className="mt-2">
...
//                           onChange={() => toggleNewQuestionCorrectOption(option)}
//                           className="mr-1"
//                         />
//                         <span>Correct</span>
//                       </div>
